        ...
        for(unsigned int j=i; j<i+seed; j++){
            ...
            while(band==true){
                ...
                for(unsigned int k=0; k<seed; k++){
                    ...
                    data_bin_comparasion[k]=data_bin_aux[k];
                    if(data_bin_comparasion[k]=='0'){
                        data_bin_comparasion[k]='1';
                        cont0++;
                     }
                    else{
                        data_bin_comparasion[k]='0';
                        cont1++;
                    }
                    cout<<data_bin_aux[k];
                }
            ...
            if(j>=4){
                ...
                data_bin_comparasion[j-i]=data_bin_aux[j-i];
                cout<<data_bin_aux[j-i];
            }
        }//fin del primer for
        data_bin_aux[seed]='\0';
        data_bin_comparasion[seed]='\0';
            ...
            if(cont0==cont1){
                ...
                for(unsigned short int k=0; k<seed; k++){
                    if(data_bin_comparasion[k+1]=='0')
                        data_bin_comparasion[k+1]='1';
                    else
                        data_bin_comparasion[k+1]='0';
                }
            }
        cout<<"\tTiene "<<cont0<<" ceros y "<<cont1<<" unos";
        cout<<"\tTransformacion "<<data_bin_comparasion<<endl;
        }
